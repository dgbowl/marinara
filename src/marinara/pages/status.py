...
dashboard_layout = html.Div(
    ...
    children=[
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(id="dash-parameters-list", children=[
                            html.Div("Select a component to view parameters.", className="text-secondary")
                        ])
                    ]
                ),
                ...
                html.Div(
                    ...
                    children=[
                        html.Div(
                            children=[
                                html.Span("Live Device Plot", className="card-header", style={"margin-bottom": 0, "border-bottom": "none", "padding-bottom": 0}),
                                dcc.Dropdown(
        ...
        html.Div(
            ...
            children=[
                html.Div("Active Pipelines & User Assignments", className="card-header"),
                html.Div(id="dash-pipelines-assignments-table", children="Loading assignments...", className="text-secondary", style={"padding": "20px"})
            ]
        )
    ]
)
...
def update_dashboard_stats(n_clicks, port, current_selector_value):
    try:
        ...
        if not ret.success:
            return "0", "0", "0", "0", [], None, html.Div("Daemon offline.", className="text-secondary")
        ...
        if jobs_ret.success:
            for job in jobs_ret.data:
                user_id = "N/A"
                if hasattr(job, "payload") and hasattr(job.payload, "user") and job.payload.user:
        ...
        for pip_name, pip in pips.items():
            status_badge = html.Span("Ready / Idle", className="badge badge-success") if pip.ready else html.Span("Offline / Busy", className="badge badge-warning")
            if pip.jobid:
                status_badge = html.Span("Executing Job", className="badge badge-primary")
                
            job_link = dcc.Link(f"Job #{pip.jobid}", href="/jobs", style={"font-weight": "600", "color": "var(--accent-color)"}) if pip.jobid else "-"
            ...
            rows.append(
                html.Tr(
                    children=[
                        html.Td(dcc.Link(pip_name, href=f"/pipelines/{port}/{pip_name}", style={"font-weight": "600", "color": "var(--accent-color)", "text-decoration": "none"})),
                        html.Td(status_badge),
                        html.Td(job_link),
                        html.Td(sample_name),
                        html.Td(owner_name, style={"font-weight": "600" if owner_name != "-" else "normal"})
                    ]
                )
            )
            
        table = html.Table(children=rows, className="stgrp")
        return str(pips_count), str(devs_count), str(drvs_count), str(cmps_count), selector_options, default_val, table
    except Exception as e:
        return "0", "0", "0", "0", [], None, html.Div(f"Error loading assignments: {str(e)}", className="text-secondary")
...
def update_dashboard_live_view(n_intervals, selected_cmp, port, historical_data, theme):
    if not selected_cmp:
        ...
        return html.Div("Select a component to view parameters.", className="text-secondary"), empty_fig, {}
    ...
    try:
            ...
            param_items.append(
                html.Div(
                    ...
                    children=[
                        ...
                        html.Span(f"{clean_value(v)}{unit_str}", className="param-item-val")
                    ]
                )
            )
            ...
            for t in uts_list:
                ...
                if cleaned_t not in historical_data["x"]:
                    ...
                    for var_name, var_info in ds["data_vars"].items():
                        ...
                        val = clean_value(var_info["data"][-1])
                        historical_data["y_data"][var_name].append(val)
        ...
        for t in historical_data["x"]:
            try:
                formatted_x.append(datetime.fromtimestamp(t).strftime('%H:%M:%S'))
            except Exception:
        ...
        return html.Div(f"Parameters temporarily unavailable.", className="text-secondary"), empty_fig, {}
    ...
    for k, v in obj.items():
        ...
        if otype in ["pipelines", "components"]:
            row[0].children = dcc.Link(
                ...
                style={"font-weight": "600", "text-decoration": "none", "color": "var(--accent-color)"}
            )
