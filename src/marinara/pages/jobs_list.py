...
def update_jobs_list(n_clicks, port):
    try:
        ...
        CTXT = zmq.Context()
        ret = ketchup.status(port=port, context=CTXT, verbosity=20, jobids=[], timeout=1000)
        if not ret.success:
            if ret.msg == "job queue is empty":
                return html.Div("No active or historical jobs found.", className="text-secondary", style={"text-align": "center", "padding": "20px"})
            return html.Div(f"No data found. Error: {ret.msg}", className="text-secondary", style={"text-align": "center", "padding": "20px"})
        ...
        for job in jobs_list:
            ...
            cleaned_jobs.append(clean_dict_values(v_dict))
        ...
        return html.Div(f"Error loading jobs: {str(e)}", className="text-secondary", style={"padding": "20px"})
